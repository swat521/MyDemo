...
public class UserResetPwd extends BaseActivity {


	private ToggleButton tb_show_hidden;
	private TextView et_reg_input_user;
	...
	private void init_user_reset_pwd() {
		// TODO Auto-generated method stub
		//set_title(R.id.tv_title, "重置密码");
		...
		LoadingDialog loadingDialog = new LoadingDialog(curContent, new LoadingDialogExecute() {
			...
			public boolean execute() {
				...
				try {
					...
					url_Params.put("user", reg_input_user);
			...
			public void executeSuccess(Bundle msg_bundle) {
				...
				try {
					...
					String sms_code = jsonObject.getString("Data");

					if (code == 0) {
						et_reg_input_yzm.setText(sms_code);
					}

					T.showLong(curContent, msg_str);
...
public void btn_reset_pwd(View v){
	reg_input_user = et_reg_input_user.getText().toString();
	...
	LoadingDialog loadingDialog = new LoadingDialog(curContent, new LoadingDialogExecute() {
		...
		public boolean execute() {
			...
			try {
				...
				url_Params = new HashMap<String, String>();
				url_Params.put("user", reg_input_user);
				url_Params.put("pwd", reg_input_pwd);
				url_Params.put("sms_code", reg_input_yzm);


				data_json = HttpUtil.doPost(resetPwdUrl, url_Params);
