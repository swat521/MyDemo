...
public class SportView  extends View {
    ...
    private float stepLong = (float) 0.00055;//每步的长度'
    private float cllPerStep = (float) 0.075;//每步消耗卡路里数量
    ...
    private void drawableText(Canvas canvas){
        ...
        //当前步数
        float bushuY = ty + getFontHeight(centerTextPaint)/2-10;
        canvas.drawText(""+progress,tx,bushuY,centerTextPaint);
        //上面的截止时间
        float topX = (this.minDistViewSize - getFontlength(textPaint,"今日步数"))/2;
        float topY = ty - dip2px(getContext(),0) - getFontHeight(centerTextPaint)/2;
        canvas.drawText("今日步数",topX,topY,textPaint);



        //公里数和卡路里
        float bottomX = (this.minDistViewSize - getFontlength(textPaint,""+(progress*stepLong)+"公里,"+Math.floor(progress*cllPerStep)+"卡"))/2;
        float bottomY = ty + getFontHeight(centerTextPaint)/2+dip2px(getContext(),24);
        canvas.drawText(""+Math.floor(progress*stepLong)+"公里,"+Math.floor(progress*cllPerStep)+"卡",bottomX,bottomY,textPaint);
