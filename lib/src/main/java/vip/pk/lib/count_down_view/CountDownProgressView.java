...
public class CountDownProgressView extends TextView {
    ...
    public CountDownProgressView(Context context, AttributeSet attrs) {
        ...
        if (typedArray != null){
            ...
            //TODO 文字颜色属性
            if (typedArray.hasValue(R.styleable.CountDownProgress_textColor3)){
                //有这个属性索引 获取用户设置的颜色
                textColor = typedArray.getColor(R.styleable.CountDownProgress_textColor3,0);
            }else{
                //没有这个属性索引 默认是白色
                textColor = typedArray.getColor(R.styleable.CountDownProgress_textColor3, Color.parseColor("#ffffff"));
            }
    ...
    public void setProgressColor(String colorStr){
        this.progressColor = Color.parseColor(colorStr);
    }
